...
def plot_cluster_alignment_matrix(
    ...
    *,
    ...
    """
    Plot a heatmap of the cluster alignment matrix.
    
    Each entry (i, j) in the matrix represents how much velocities in cluster i
    point toward cluster j's state.
    
    Parameters
    ----------
    alignment_matrix : pd.DataFrame
        Cluster alignment matrix DataFrame (typically from compute_cluster_alignment_matrix).
        Rows are source clusters, columns are target clusters.
    figsize : Tuple[float, float], default (10, 8)
        Figure size (width, height) in inches.
    cmap : str, default 'RdBu_r'
        Colormap for the heatmap. 'RdBu_r' maps low values to blue and high values to red.
    ...
    center : float, optional
        Center value for diverging colormap. If None, no centering.
        Useful for non-normalized matrices to center at 0.
    title : str, optional
    ...
    show_plot : bool, default True
        If True, calls plt.show() at the end.
    save_plot : bool, default False
        If True, saves the figure to `save_path` (PNG).
    save_path : str, optional
        Path to save PNG. Only used if `save_plot` is True.
        If None, uses a default filename.
    ...
    Examples
    --------
    >>> # Compute alignment matrix first
    >>> df = vae.compute_cluster_alignment_matrix(use_gp_space=True, normalize=True)
    >>> 
    >>> # Then plot it
    >>> fig, ax = lineagevi.plots.plot_cluster_alignment_matrix(df)
    >>> 
    >>> # Customize appearance
    >>> fig, ax = lineagevi.plots.plot_cluster_alignment_matrix(
    ...     df,
    ...     cmap='viridis',
    ...     figsize=(12, 10),
    ...     title='Custom Alignment Matrix'
    ... )
    """
    # Prepare figure/axes
    created_fig = False
    ...
    # Style: move x-axis ticks and labels to top
    ax.xaxis.tick_top()
    ax.xaxis.set_label_position('top')
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=90)
    
    # Check if matrix is symmetric (within numerical precision)
    # Only show Source/Target labels if matrix is not symmetric
    is_symmetric = np.allclose(alignment_matrix.values, alignment_matrix.values.T, rtol=1e-10, atol=1e-10)
    if not is_symmetric:
        # Set axis labels only for non-symmetric matrices
        ax.set_xlabel('Target', fontsize=title_fontsize)
        ax.set_ylabel('Source', fontsize=title_fontsize)
    ...
    if save_plot:
        if save_path is None:
            save_path = "cluster_alignment_matrix.png"
